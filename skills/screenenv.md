...
### Check Job Status
```bash
kubectl get job screenenv-{demo_id} -n infra --context your-k8s-context
```

### View Logs
```bash
kubectl logs job/screenenv-{demo_id} -n infra --context your-k8s-context
```
...
POD=$(kubectl get pods -n infra -l job-name=screenenv-{demo_id} \
  --context your-k8s-context -o jsonpath='{.items[0].metadata.name}')

# Check file exists
kubectl exec -n infra $POD --context your-k8s-context -- \
